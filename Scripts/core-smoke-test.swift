...
expectEqual(presentation.previousText, "", "the first caption has no history")
...
expectEqual(presentation.previousText, "", "revisions do not become separate history entries")
expectEqual(presentation.update("Unused", utteranceID: 2, isFinal: false), "Unused", "a new utterance starts independently")
expectEqual(presentation.previousText, "Credits offset costs.", "the prior caption remains visible when streaming the next")
_ = presentation.update("Unused credits expire.", utteranceID: 2, isFinal: false)
expectNil(presentation.update("Unused credits expire.", utteranceID: 2, isFinal: true), "identical final text is still committed without a redraw")
_ = presentation.update("Third caption.", utteranceID: 3, isFinal: true)
expectEqual(presentation.previousText, "Unused credits expire.", "history keeps exactly one prior caption, including identical stream finals")
_ = presentation.update("Interrupted stream", utteranceID: 4, isFinal: false)
_ = presentation.update("Fifth caption.", utteranceID: 5, isFinal: true)
expectEqual(presentation.previousText, "Third caption.", "a failed stream does not replace the last finished caption")
expectNil(presentation.update("  ", utteranceID: 6, isFinal: true), "empty results cannot advance history")
expectEqual(presentation.previousText, "Third caption.", "empty results preserve history")
expectEqual(presentation.update("Fifth caption.", utteranceID: 6, isFinal: true), "Fifth caption.", "repeated wording in a new utterance still advances history")
expectEqual(presentation.previousText, "Fifth caption.", "new utterance IDs, not text equality, define history")
