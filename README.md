...
- `INTERVAL`: 자막 영역을 읽는 주기입니다. 기본값은 `0.4`초이며, 더 빠른 반응이 필요하면 `0.2-0.4`, 안정적인 인식을 우선하면 `0.5-0.8` 정도를 권장합니다.
- `STABLE_AFTER`: 같은 자막이 유지된 뒤 번역을 시작하기까지의 대기 시간입니다. 기본값은 `1.2`초이며, 빠른 응답은 `0.6-1.0`, 정확도 우선은 `1.2-2.0` 정도가 적절합니다.
